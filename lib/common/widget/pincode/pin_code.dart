...
class PinCode extends StatefulWidget {
  ...
  /// the style of the text, default is [ fontSize: 20,
  ...
  /// Auto dismiss the keyboard upon inputting the
  /// value for the last field. Default is [true]
  final bool autoDismissKeyboard;

  /// Auto dispose the [controller] and [FocusNode] upon
  /// the destruction of widget from the widget tree. Default is [true]
  final bool autoDisposeControllers;

  /// Configures how the platform keyboard will select an
  /// uppercase or lowercase keyboard.
  /// Only supports text keyboards, other keyboard types will
  /// ignore this configuration. Capitalization is locale-aware.
  ...
  PinCode({
    ...
    this.pinTheme = const MyPinTheme.defaults(),
...
class _PinCodeState extends State<PinCode> with SingleTickerProviderStateMixin {
  ...
  MyPinTheme get _pinTheme => widget.pinTheme ?? MyPinTheme();
  ...
  void initState() {
    ...
    _assignController();
    if (_pinTheme.shape != MyPinCodeFieldShape.circle &&
        _pinTheme.shape != MyPinCodeFieldShape.underline) {
      ...
      if (widget.enabled && _inputList.join('') != currentText) {
        if (currentText.length >= widget.length) {
          if (widget.onCompleted != null) {
            ...
            //  delay the onComplete event handler to give the onChange
            //  event handler enough time to complete
  ...
  void dispose() {
    if (widget.autoDisposeControllers) {
      ...
      //       To disable this feature please set
              ...
              child: AutofillGroup(
                child: TextFormField(
                  ...
                  style: const TextStyle(
                    ...
                    fontSize: kIsWeb
                        ? 1
                        : 0.01, // it is a hidden textfield which should
        ...
        AnimatedContainer(
          ...
          decoration: BoxDecoration(
            color: widget.enableActiveFill
                ...
                : Colors.transparent,
            shape: _pinTheme.shape == MyPinCodeFieldShape.circle
                ? BoxShape.circle
                : BoxShape.rectangle,
            borderRadius: borderRadius,
            border: _pinTheme.shape == MyPinCodeFieldShape.underline
                ? Border(
                    bottom: BorderSide(
...
enum MyPinCodeFieldShape { box, underline, circle }
