...
namespace ARLocation
{
    ...
    public class PlaceAtLocation : MonoBehaviour
    {
        ...
        public double LimitDistance = 300;
        ...
        void Initialize()
        {
            ...
            if (!hasInitialized)
            {
                ...
                {
                    smoothMove = SmoothMove.AddSmoothMove(gameObject, PlacementOptions.MovementSmoothing, maxDistance: LimitDistance);
                }
        ...
        public void UpdatePosition(Location deviceLocation, bool forceUpdate = false)
        {
            ...
            {
                //float distance = Vector3.Distance(mainCameraTransform.position, targetPosition);
                //print("mainCameraTransform.position " + Vector3.Distance(mainCameraTransform.position, targetPosition));
                //if (distance > LimitDistance)
                //{
                //    transform.localScale = new Vector3(ConfigUtil.ScaleLocationMin, ConfigUtil.ScaleLocationMin, ConfigUtil.ScaleLocationMin);
                //    //transform.position = (targetPosition - mainCameraTransform.position).normalized * 50 ;
                //}
                //else
                //{
                ...
                //}
                float scaleFactor = Vector3.Distance(targetPosition, mainCameraTransform.position) * 0.125f;
                scaleFactor = Mathf.Clamp(scaleFactor, 0, 10000);
